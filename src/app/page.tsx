...
export default function GuiaPage() {
  ...
  const [activeView, setActiveView] = useState<'zones' | 'categories' | 'list' | 'urgencias'>('zones');
  ...
  React.useEffect(() => {
    supabase.from('guide_categories').select('*').then((response) => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  return (
    <main className="bg-background text-on-surface font-body selection:bg-primary selection:text-on-primary relative overflow-x-hidden min-h-screen pb-24">
      ...
      <header className="fixed top-0 w-full z-50 bg-[#131313]/70 backdrop-blur-xl shadow-[0_20px_50px_rgba(229,226,225,0.05)]">
        <div className="flex justify-between items-center px-6 h-16 w-full max-w-7xl mx-auto">
          <div className="flex items-center gap-4">
            <button 
              onClick={() => {
                if (activeView === 'list') setActiveView('categories');
      ...
      <div className="pt-24 px-6 max-w-7xl mx-auto relative z-10 w-full animate-in fade-in duration-500">
        ...
        <section className="mb-8 relative overflow-hidden rounded-xl h-[250px] flex flex-col justify-end p-8 shadow-2xl border border-[#d2b86b]/20">
          <div className="absolute inset-0 z-0">
            <div className="absolute inset-0 bg-linear-to-t from-[#131313] via-[#131313]/60 to-transparent"></div>
          </div>
          <div className="relative z-10 text-center md:text-left">
            <h1 className="font-headline text-3xl md:text-5xl font-black tracking-tight mb-2 text-white">Explora tu Zona</h1>
            <p className="font-body text-[#d2b86b] text-sm tracking-widest uppercase font-bold">Selecciona tu ubicación actual</p>
          </div>
        </section>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {ZONES.map((zone) => (
            <div 
              ...
              className="group relative overflow-hidden rounded-xl bg-[#1e1e1e] min-h-[120px] border border-white/5 cursor-pointer flex items-center justify-center text-center p-6 hover:border-[#d2b86b]/50 transition-all duration-300"
            >
              <h3 className="font-headline font-bold text-xl tracking-wider text-white group-hover:text-[#d2b86b] transition-colors relative z-10">{zone}</h3>
            </div>
          ))}
        </section>
      </div>
      ...
      <nav className="fixed bottom-0 w-full bg-[#131313]/90 backdrop-blur-xl border-t border-white/5 py-4 pb-8 z-50 flex justify-center gap-8 px-6">
        <button onClick={() => { setActiveView('zones'); window.scrollTo(0, 0); }} className={`flex flex-col items-center gap-1 transition-colors ${activeView==='zones'||activeView==='categories'||activeView==='list' ? 'text-[#d2b86b]' : 'text-gray-500 hover:text-[#d2b86b]'}`}>
